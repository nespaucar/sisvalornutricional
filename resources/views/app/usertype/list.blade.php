	...
	<table id="example1" class="table table-condensed table-hover">
		...
		<tbody>
			...
			<tr>
				<td>{{ $contador }}</td>
				<td>{{ $value->nombre }}</td>
				<td class="text-center">{!! Form::button('<div class="glyphicon glyphicon-list"></div> Permisos', array('onclick' => 'modal(\''.URL::route($ruta["permisos"], array('SI', $value->id)).'\', \'Permisos para '.$value->nombre.'\', this);', 'class' => 'btn btn-default btn-xs waves-effect waves-light')) !!}</td>
